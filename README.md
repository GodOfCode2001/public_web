...
写在前面
你在本地写了一个网页、跑了一个服务，输入 http://localhost:3000 能打开，但想让朋友点开链接就能访问——关键是你家宽带没有公网IP，运营商给你的是内网地址。
...
左侧菜单点击 Workers & Pages → 点击 创建应用 → 选择 Pages
...
部署完成后，Pages 会给你一个 .pages.dev 的二级域名，比如 my-project.pages.dev

点开就能访问，全球 CDN 加速

第三步（可选）：绑定你自己的域名
在 Pages 项目设置里找到 自定义域

点击 设置自定义域，输入你的域名（比如 guoyu.v6.rocks）

Cloudflare 会自动帮你配置 DNS，等待生效即可
...
第一步：创建 Worker
Cloudflare Dashboard → Workers & Pages → 创建应用 → Workers

选择模板或空白模板

在线编辑器中写代码，比如一个简单的 API：
...
你的代码 → Git推送 或 直接上传
    ...
    Cloudflare Pages（静态托管）
              ↓
    Cloudflare Workers（API/逻辑）
              ↓
    全球边缘节点（CDN加速）
              ↓
    用户访问：your-domain.com 或 xxx.pages.dev
...
用户访问 your-domain.com
  ...
  Cloudflare Tunnel（加密隧道）
        ↓
   你的电脑 localhost:3000
关键点：隧道是你的电脑主动发起的出站连接，所以不需要公网IP，也不需要路由器开端口。
...
输入你想要的前缀（比如 guoyu），选择后缀 v6.rocks

dynv6 提供多个免费后缀：v6.rocks、v6.army、v6.navy 等

点击创建，域名就归你了

进入域名详情页，复制你的 Token（后面要用，但我们的场景里用不到它，因为不是用 DDNS 更新 IP，而是把域名托管给 Cloudflare）
...
点击 添加站点，输入 guoyu.v6.rocks

Cloudflare 会扫描现有 DNS 记录，扫描完点击继续

选择 免费套餐

Cloudflare 会给你两个 Nameserver 地址（类似 abby.ns.cloudflare.com）

回到 dynv6.com，点击你的域名进入详情页

找到 “Nameserver” 设置，改成 Cloudflare 给你的那两个地址

等待 DNS 生效（通常 10-30 分钟，最长 24 小时）
...
新建一个 .bat 文件，内容：cloudflared tunnel --hostname guoyu.v6.rocks --url http://localhost:3000

打开任务计划程序 → 创建任务 → 触发器选“开机时” → 操作选启动这个 .bat 文件

第六步：配置本地服务自动启动（可选）
如果你的本地 Web 服务（比如 Node.js、Python Flask）也需要开机自启，建议配合 PM2（Node）或 systemd（Linux）一起使用，保证电脑重启后整套系统自动恢复。
...
A：隧道是加密的（TLS 1.3），而且 Cloudflare 默认开启 DDoS 防护和 WAF（Web应用防火墙），比直接暴露公网IP更安全。
...
Q：Pages 部署后更新怎么办？
A：如果连接了 Git 仓库，每次 push 代码会自动重新部署。如果是直接上传的文件夹，重新上传覆盖即可。

最后的最后
无论选哪个方案，从 localhost 到公网可访问，都只需要 5-15 分钟。选一个适合你的方式，动手试起来吧！
